...
if ( ! function_exists( 'ashtanga_core_add_wishlist_profile_navigation_item' ) ) {
	function ashtanga_core_add_wishlist_profile_navigation_item( $items, $dashboard_url ) {
		$items['my-wishlist'] = array(
			'url'         => esc_url( add_query_arg( array( 'user-action' => 'my-wishlist' ), $dashboard_url ) ),
			'text'        => esc_html__( 'My Wishlist', 'ashtanga-core' ),
	...
	add_filter( 'ashtanga_membership_filter_dashboard_navigation_action_pages', 'ashtanga_core_add_wishlist_profile_navigation_item', 10, 2 );
}

if ( ! function_exists( 'ashtanga_core_add_wishlist_profile_navigation_pages' ) ) {
	function ashtanga_core_add_wishlist_profile_navigation_pages( $html, $action ) {

		if ( 'my-wishlist' === $action ) {
			$atts = array();

			$html = ashtanga_core_get_template_part( 'wishlist', '/profile/templates/my-wishlist', '', $atts );
		}

		return $html;
	}

	add_filter( 'ashtanga_membership_filter_dashboard_page', 'ashtanga_core_add_wishlist_profile_navigation_pages', 10, 2 );
}
