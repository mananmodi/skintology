...
if ( ! function_exists( 'ashtanga_core_add_custom_font_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function ashtanga_core_add_custom_font_shortcode( $shortcodes ) {
		$shortcodes[] = 'AshtangaCore_Custom_Font_Shortcode';

		return $shortcodes;
	}

	add_filter( 'ashtanga_core_filter_register_shortcodes', 'ashtanga_core_add_custom_font_shortcode' );
}

if ( class_exists( 'AshtangaCore_Shortcode' ) ) {
	class AshtangaCore_Custom_Font_Shortcode extends AshtangaCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'ashtanga_core_filter_custom_font_layouts', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( ASHTANGA_CORE_SHORTCODES_URL_PATH . '/custom-font' );
			$this->set_base( 'ashtanga_core_custom_font' );
			$this->set_name( esc_html__( 'Custom Font', 'ashtanga-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays custom font with provided parameters', 'ashtanga-core' ) );
			...
			$this->set_option(
				array(
					'field_type'    => 'textarea',
					'name'          => 'title',
					'title'         => esc_html__( 'Title Text', 'ashtanga-core' ),
					'default_value' => esc_html__( 'Custom Title Text', 'ashtanga-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'options'       => ashtanga_core_get_select_type_options_pool( 'appear_animation', false ),
					'default_value' => 'none',
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Custom Parallax Effect', 'ashtanga-core' ),
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Parallax Amount', 'ashtanga-core' ),
					'description' => esc_html__( 'Enter the number that represents the pixels  of parallax move.', 'ashtanga-core' ),
					'dependency'  => array(
						'show' => array(
			...
			$this->set_option(
				array(
					'field_type'  => 'text',
					'name'        => 'parallax_amount_touch',
					'title'       => esc_html__( 'Parallax Amount Touch', 'ashtanga-core' ),
					'description' => esc_html__( 'Enter the number that represents the pixels of parallax move on touch devices.', 'ashtanga-core' ),
					'dependency'  => array(
						'show' => array(
							'custom_parallax' => array(
								'values'        => 'yes',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Parallax Direction', 'ashtanga-core' ),
					'options'    => array(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'ashtanga_core_custom_font', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			$atts['parallax_touch_data'] = $this->get_parallax_touch_data( $atts );
			$this->set_responsive_styles( $atts );

			return ashtanga_core_get_template_part( 'shortcodes/custom-font', 'variations/' . $atts['layout'] . '/templates/custom-font', '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ! empty( $atts['parallax_direction'] ) ? 'qodef-parallax-direction--' . $atts['parallax_direction'] : '';
			$holder_classes[] = ! empty( $atts['custom_parallax'] ) && ( 'yes' === $atts['custom_parallax'] ) ? 'qodef-custom-parallax' : '';
			$holder_classes[] = ( 'none' !== $atts['appear_animation'] ) ? 'qodef--has-appear qodef--appear-' . $atts['appear_animation'] : '';
		...
		private function get_parallax_touch_data( $atts ) {
			$data = array();

			if ( ! empty( $atts['parallax_amount_touch'] ) ) {
				$data[] = $atts['parallax_amount_touch'];
			}
