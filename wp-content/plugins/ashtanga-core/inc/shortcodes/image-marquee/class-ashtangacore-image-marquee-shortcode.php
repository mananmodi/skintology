...
if ( ! function_exists( 'ashtanga_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function ashtanga_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'AshtangaCore_Image_Marquee_Shortcode';

		return $shortcodes;
	}

	add_filter( 'ashtanga_core_filter_register_shortcodes', 'ashtanga_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'AshtangaCore_Shortcode' ) ) {
	class AshtangaCore_Image_Marquee_Shortcode extends AshtangaCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'ashtanga_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'ashtanga_core_filter_image_marquee_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( ASHTANGA_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'ashtanga_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'ashtanga-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'ashtanga-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type'    => 'text',
					'name'          => 'duration',
					'title'         => esc_html__( 'Animation Duration (Seconds)', 'ashtanga-core' ),
					'default_value' => '20',
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return ashtanga_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
