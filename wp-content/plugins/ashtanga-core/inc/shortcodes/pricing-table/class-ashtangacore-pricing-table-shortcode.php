...
if ( ! function_exists( 'ashtanga_core_add_pricing_table_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function ashtanga_core_add_pricing_table_shortcode( $shortcodes ) {
		$shortcodes[] = 'AshtangaCore_Pricing_Table_Shortcode';

		return $shortcodes;
	}

	add_filter( 'ashtanga_core_filter_register_shortcodes', 'ashtanga_core_add_pricing_table_shortcode' );
}

if ( class_exists( 'AshtangaCore_Shortcode' ) ) {
	class AshtangaCore_Pricing_Table_Shortcode extends AshtangaCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'ashtanga_core_filter_pricing_table_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'ashtanga_core_filter_pricing_table_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( ASHTANGA_CORE_SHORTCODES_URL_PATH . '/pricing-table' );
			$this->set_base( 'ashtanga_core_pricing_table' );
			$this->set_name( esc_html__( 'Pricing Table', 'ashtanga-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds pricing table element', 'ashtanga-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'featured_table',
					'title'         => esc_html__( 'Featured Table', 'ashtanga-core' ),
			...
			$this->set_option(
				array(
					...
					'group'      => esc_html__( 'Content Style', 'ashtanga-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'default_value' => esc_html__( 'Subtitle Text', 'ashtanga-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type'    => 'text',
					'name'          => 'price',
					'title'         => esc_html__( 'Price', 'ashtanga-core' ),
					'default_value' => esc_html__( '99', 'ashtanga-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'color',
					'name'       => 'price_color',
					'title'      => esc_html__( 'Price Color', 'ashtanga-core' ),
					'group'      => esc_html__( 'Price Style', 'ashtanga-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type'    => 'text',
					'name'          => 'currency',
					'title'         => esc_html__( 'Currency', 'ashtanga-core' ),
					'default_value' => esc_html__( '$', 'ashtanga-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'color',
					'name'       => 'currency_color',
					'title'      => esc_html__( 'Currency Color', 'ashtanga-core' ),
					'group'      => esc_html__( 'Currency Style', 'ashtanga-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public function render( $options, $content = null ) {
			...
			$atts['action_holder_styles'] = $this->get_action_holder_styles( $atts );
			...
			return ashtanga_core_get_template_part( 'shortcodes/pricing-table', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_action_holder_styles( $atts ) {
			$styles = array();
