...
if ( ! function_exists( 'ashtanga_core_add_banner_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function ashtanga_core_add_banner_shortcode( $shortcodes ) {
		$shortcodes[] = 'AshtangaCore_Banner_Shortcode';

		return $shortcodes;
	}

	add_filter( 'ashtanga_core_filter_register_shortcodes', 'ashtanga_core_add_banner_shortcode' );
}

if ( class_exists( 'AshtangaCore_Shortcode' ) ) {
	class AshtangaCore_Banner_Shortcode extends AshtangaCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'ashtanga_core_filter_banner_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'ashtanga_core_filter_banner_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( ASHTANGA_CORE_SHORTCODES_URL_PATH . '/banner' );
			$this->set_base( 'ashtanga_core_banner' );
			$this->set_name( esc_html__( 'Banner', 'ashtanga-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds banner element', 'ashtanga-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'subtitle_tag',
					'title'         => esc_html__( 'Subtitle Tag', 'ashtanga-core' ),
					'options'       => ashtanga_core_get_select_type_options_pool( 'title_tag' ),
					'default_value' => 'h5',
		...
		public function render( $options, $content = null ) {
			...
			return ashtanga_core_get_template_part( 'shortcodes/banner', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
