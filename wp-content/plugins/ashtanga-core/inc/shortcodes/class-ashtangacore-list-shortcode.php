...
abstract class AshtangaCore_List_Shortcode extends QodeFrameworkShortcode {
	private $post_type;
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'ashtanga_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );
	...
	public function map_list_options( $params = array() ) {
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behavior', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => ashtanga_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => array(
						''      => esc_html__( 'Original', 'ashtanga-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => ashtanga_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
					'default_value' => '3',
					'group'         => $group,
					'dependency'    => array(
						'hide' => array(
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'split_column',
					'title'         => esc_html__( 'Slider split columns', 'ashtanga-core' ),
					'description'         => esc_html__( 'Allowing this option in the slider will be shown a half of next slide.', 'ashtanga-core' ),
		...
		$this->set_option(
			array(
				...
				'title'         => esc_html__( 'Custom Items Vertical Spacing - 680', 'ashtanga-core' ),
				'default_value' => '',
				'dependency'    => array(
					'show' => array(
						'vertical_space' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Row Height', 'ashtanga-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Max Row Height', 'ashtanga-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Last Row Treshold', 'ashtanga-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option(
			array(
				'field_type'  => 'text',
				'name'        => 'slider_speed',
				'title'       => esc_html__( 'Slide Duration', 'ashtanga-core' ),
				'description' => esc_html__( 'Default value is 5000 (ms)', 'ashtanga-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
		$this->set_option(
			array(
				...
				'title'       => esc_html__( 'Slide Animation Duration', 'ashtanga-core' ),
				'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'ashtanga-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
		...
		$this->set_option(
			array(
				...
				'title'         => esc_html__( 'Choose Slide Animation', 'ashtanga-core' ),
				'options'       => array(
					''     => esc_html__( 'Default (Slide)', 'ashtanga-core' ),
					'fade' => esc_html__( 'Fade', 'ashtanga-core' ),
				),
				'default_value' => '',
				'description'   => esc_html__( 'Number of columns will be set to one.', 'ashtanga-core' ),
				'dependency'    => $dependency,
				'group'         => $group,
			)
		);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_direction', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Choose Slide Direction', 'ashtanga-core' ),
					'options'       => array(
						''         => esc_html__( 'Default (Horizontal)', 'ashtanga-core' ),
						'vertical' => esc_html__( 'Vertical', 'ashtanga-core' ),
					),
					'default_value' => '',
					'description'   => esc_html__( 'Number of columns will be set to one.', 'ashtanga-core' ),
					'dependency'    => $dependency,
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_hidden_slides', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Show Hidden Slides', 'ashtanga-core' ),
					'options'     => ashtanga_core_get_select_type_options_pool( 'yes_no' ),
					'description' => esc_html__( 'Works best with horizontal slide direction and slide animation.', 'ashtanga-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Loop Additional Slides', 'ashtanga-core' ),
					'description' => esc_html__( 'Default value is 0 and this option works only when Show Hidden Slides option is enabled', 'ashtanga-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_centered_slides', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Enable Centered Slides', 'ashtanga-core' ),
					'options'     => ashtanga_core_get_select_type_options_pool( 'yes_no' ),
					'description' => esc_html__( 'If true, then active slide will be centered, not always on the left side.', 'ashtanga-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_mousewheel_navigation', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Enable Slider Mousewheel Navigation', 'ashtanga-core' ),
					'options'     => ashtanga_core_get_select_type_options_pool( 'yes_no' ),
					'description' => esc_html__( 'Enables navigation through slides using mouse wheel.', 'ashtanga-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_drag_cursor', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Show Drag Cursor', 'ashtanga-core' ),
					'options'    => ashtanga_core_get_select_type_options_pool( 'yes_no' ),
					'dependency' => $dependency,
					'group'      => $group,
				)
		...
		$this->set_option(
			array(
				...
				'title'      => esc_html__( 'Show Slider Navigation', 'ashtanga-core' ),
				'options'    => ashtanga_core_get_select_type_options_pool( 'yes_no', true, '', array( 'combo' => esc_html__( 'Combo', 'ashtanga-core' ) ) ),
				'dependency' => $dependency,
				'group'      => $group,
			)
		);
		...
		$this->set_option(
			array(
				...
				'title'       => esc_html__( 'Show Slider Pagination', 'ashtanga-core' ),
				'options'     => ashtanga_core_get_select_type_options_pool( 'yes_no' ),
				'description' => esc_html__( 'This option does\'t work when Show Slider Navigation option is set to Combo', 'ashtanga-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
	...
	public function get_list_classes( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && 'slider' == $atts['behavior'] ) {

			if ( ! empty( $atts['slider_layout'] ) ) {
				$holder_classes[] = 'qodef-slider-layout--' . esc_attr( $atts['slider_layout'] );
			}
		}
	...
	public function get_list_styles( $atts ) {
		$styles = ashtanga_core_get_gutter_custom_styles( 'space_', 'vertical_space_', $atts, true );
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && 'masonry' === $atts['behavior'] && ! empty( $atts['masonry_images_proportion'] ) && 'fixed' === $atts['masonry_images_proportion'] ) {
			...
			$image_dimension              = ashtanga_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		if ( isset( $atts['space'] ) ) {
			if ( 'custom' === $atts['space'] ) {
				...
				$data['spaceBetween'] = ashtanga_core_get_space_value( $atts['space'] );
			}
		} else {
			$data['spaceBetween'] = 0;
		}
		...
		if ( isset( $atts['slider_layout'] ) ) {

			switch ( $atts['slider_layout'] ) {

				case 'centered-custom':
					$data['centeredSlides'] = true;
					$data['slidesPerView']  -= 0.5;
					break;

				case 'centered-original':
					$data['centeredSlides'] = true;
	...
	public
	function map_query_options(
		$params = array()
	) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'ashtanga-core' );
		...
		$this->set_option(
			array(
				'field_type'    => 'text',
				'name'          => 'posts_per_page',
				'title'         => esc_html__( 'Posts per Page', 'ashtanga-core' ),
				'default_value' => '9',
				'group'         => $group,
			)
		);
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'orderby', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'orderby',
					'title'         => esc_html__( 'Order By', 'ashtanga-core' ),
					'options'       => ashtanga_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
					'default_value' => 'date',
					'group'         => $group,
				)
			);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'order', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'order',
					'title'         => esc_html__( 'Order', 'ashtanga-core' ),
					'options'       => ashtanga_core_get_select_type_options_pool( 'order' ),
					'default_value' => 'DESC',
					'group'         => $group,
				)
			);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option, true ) ) ) {
			...
			do_action( 'ashtanga_core_action_map_query_options_before_additional', $group );

			$additional_params = apply_filters(
				'ashtanga_core_filter_map_additional_query_params',
				array(
					''       => esc_html__( 'No', 'ashtanga-core' ),
					'id'     => esc_html__( 'Post IDs', 'ashtanga-core' ),
					'tax'    => esc_html__( 'Taxonomy Slug', 'ashtanga-core' ),
					'author' => esc_html__( 'Author Name', 'ashtanga-core' ),
				),
				$this->get_base()
			);
			...
			$this->set_option(
				array(
					'field_type'  => 'text',
					'name'        => 'post_ids',
					'title'       => esc_html__( 'Posts IDs', 'ashtanga-core' ),
					'description' => esc_html__( 'Separate post IDs with commas', 'ashtanga-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				$this->set_option(
					array(
						'field_type' => 'select',
						'name'       => 'tax',
						'title'      => esc_html__( 'Taxonomy', 'ashtanga-core' ),
				...
				$this->set_option(
					array(
						'field_type' => 'text',
						'name'       => 'tax_slug',
						'title'      => esc_html__( 'Taxonomy Slug', 'ashtanga-core' ),
						'group'      => $group,
						'dependency' => array(
							'show' => array(
								'additional_params' => array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'author_slug',
					'title'      => esc_html__( 'Author Slug', 'ashtanga-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
			...
			do_action( 'ashtanga_core_action_map_query_options_after_additional', $group );
		}
	}

	public
	function get_post_type_additional_taxonomies() {
		return $this->post_type_additional_taxonomies;
	}

	public
	function set_post_type_additional_taxonomies(
		$post_type_additional_taxonomies
	) {
		$this->post_type_additional_taxonomies = $post_type_additional_taxonomies;
	}

	public
	function get_additional_query_args(
		$atts
	) {
		$args = array();
		...
		$args = apply_filters( 'ashtanga_core_filter_additional_query_args', $args, $atts, $this->get_post_type() );
	...
	function map_layout_options(
		...
		$this->set_option(
			array(
				...
				'options'       => $layouts,
				'default_value' => apply_filters( 'ashtanga_core_filter_map_layout_options_default_value', $default_value, $this->get_base(), $params ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'text_transform',
					'title'      => esc_html__( 'Title Text Transform', 'ashtanga-core' ),
	...
	public
	function map_additional_options(
		$params = array()
	) {
		$group_name         = esc_html__( 'Additional', 'ashtanga-core' );
		...
		if ( ! $exclude_filter ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'enable_filter',
					'title'         => esc_html__( 'Enable Filter', 'ashtanga-core' ),
					'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'ashtanga-core' ),
					'options'       => ashtanga_core_get_select_type_options_pool( 'no_yes' ),
					'default_value' => '',
					'dependency'    => array(
						'show' => array(
			...
			do_action( 'ashtanga_core_action_map_additional_options_after_filter', $group_name );
		}
		...
		if ( ! $exclude_pagination ) {
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Pagination Top Margin (px or %)', 'ashtanga-core' ),
					'default_value' => '',
					'group'         => $group_name,
					'dependency'    => array(
						'show' => array(
			...
			do_action( 'ashtanga_core_action_map_additional_options_after_pagination', $group_name );
		}
	}

	public
	function map_extra_options() {
	...
	function load_assets() {
		do_action( 'ashtanga_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
