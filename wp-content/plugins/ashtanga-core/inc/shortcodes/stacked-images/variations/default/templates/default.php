	...
	<div class="qodef-m-images">
		<?php if ( ! empty( $main_image ) ) { ?>
			<div class="qodef-m-image">
				...
				echo ashtanga_core_get_list_shortcode_item_image( $images_proportion, $main_image, $custom_image_width, $custom_image_height );
				?>
			</div>
		<?php } ?>
		<?php if ( ! empty( $items ) ) {
			foreach ( $items as $item ) { ?>
				<div <?php qode_framework_class_attribute( $item_classes ); ?> style="<?php echo esc_attr($item['item_vertical_anchor']) . ':' . esc_attr($item['item_vertical_position']) . '; ' . esc_attr($item['item_horizontal_anchor']) . ':' . esc_attr($item['item_horizontal_position']) . '; ' . 'text-align' . ':' . esc_attr($item['item_horizontal_anchor']); ?>">
					...
					echo ashtanga_core_get_list_shortcode_item_image( $item['images_proportion'], $item['item_image'], $item['custom_image_width'], $item['custom_image_height'] );
					?>
				</div>
			<?php }
		} ?>
