(function ( $ ) {
    ...
    var qodefImport = {
        ...
        importAttachments: function ( repeat ) {
            ...
            if ( ! repeat ) {
                for ( var i = 1; i < 20; i++ ) {
                    var xml  = i < 20 ? 'ashtanga_content_' + i + '.xml' : 'ashtanga_content_' + i + '.xml';
                    var data = {
                        action: 'content',
                        xml: xml,
                        images: qodefImport.importImages,
            ...
            $.ajax(
                {
                    ...
                    success: function ( response ) {
                        if ( response.status === 'success' ) {
                            ...
                            var purchaseKey = $( '#qode-register-purchase-key' );

                            purchaseKey.removeClass( 'qodef-cd-button-disabled' );
                            purchaseKey.attr( 'disabled', false );
                            purchaseKey.siblings( '.qodef-cd-button-wait' ).hide();
