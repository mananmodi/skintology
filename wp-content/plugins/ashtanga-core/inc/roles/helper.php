...
if ( ! function_exists( 'ashtanga_core_include_role_custom_fields' ) ) {
	/**
	 * Function that includes role custom fields files
	 */
	function ashtanga_core_include_role_custom_fields() {
		foreach ( glob( ASHTANGA_CORE_INC_PATH . '/roles/*/role-fields.php' ) as $role_fields ) {
			include_once $role_fields;
		}
	}

	add_action( 'qode_framework_action_custom_user_fields', 'ashtanga_core_include_role_custom_fields' );
}

if ( ! function_exists( 'ashtanga_core_register_role_custom_fields' ) ) {
	/**
	 * Function that registers role custom fields files
	 */
	function ashtanga_core_register_role_custom_fields() {
		do_action( 'ashtanga_core_action_register_role_custom_fields' );
	}

	add_action( 'qode_framework_action_custom_user_fields', 'ashtanga_core_register_role_custom_fields', 11 );
}

if ( ! function_exists( 'ashtanga_core_profile_get_social_params' ) ) {
	/**
	 * Returns owner params
	 */
	function ashtanga_core_profile_get_social_params( $params ) {
	...
	add_filter( 'ashtanga_membership_filter_user_params', 'ashtanga_core_profile_get_social_params' );
}

if ( ! function_exists( 'ashtanga_core_social_edit_profile_fields' ) ) {
	function ashtanga_core_social_edit_profile_fields( $page, $params ) {
		$roles_social_scope = apply_filters( 'ashtanga_core_filter_role_social_array', array( 'administrator', 'author' ) );
		$enable             = false;
	...
	add_action( 'ashtanga_membership_action_after_dashboard_edit_profile_fields', 'ashtanga_core_social_edit_profile_fields', 15, 2 );
}

if ( ! function_exists( 'ashtanga_core_user_profile_social_update' ) ) {
	function ashtanga_core_user_profile_social_update( $options, $user_id ) {
	...
	add_action( 'ashtanga_membership_action_update_user_profile', 'ashtanga_core_user_profile_social_update', 10, 2 );
}

if ( ! function_exists( 'ashtanga_core_add_user_custom_fields' ) ) {
	/**
	 * Function creates custom fields for users
	 *
	 * return $user_contact
	 */
	function ashtanga_core_add_user_custom_fields( $user_contact ) {
		/**
		 * Function that add custom user fields
		 **/
		$user_contact['position'] = esc_html__( 'Degree', 'ashtanga-core' );

		return $user_contact;
	}

	add_filter( 'user_contactmethods', 'ashtanga_core_add_user_custom_fields' );
}
