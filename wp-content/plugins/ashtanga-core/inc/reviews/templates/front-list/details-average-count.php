<?php if ( is_array( $post_ratings ) && count( $post_ratings ) ) {
	$average_rating_total = intval( round( ashtanga_core_get_total_average_rating( $post_ratings ) ) );
	?>
				...
				<span class="qodef-stars-items">
					...
					for ( $i = 1; $i <= 5; $i ++ ) {
						if ( $average_rating_total >= $i ) {
							echo ashtanga_core_get_svg_icon( 'star', 'qodef-active' );
						} else {
							echo ashtanga_core_get_svg_icon( 'star', 'qodef-inactive' );
						}
					}
					?>
				</span>
