	...
	var qodefElementorSectionCallback = {
						...
						properties.forEach(
							function ( property ) {
								...
								if ( typeof property['offset_type'] !== 'undefined' && property['offset_type'] === 'offset' ) {
									...
									settings['offset_retina']            = property['offset_retina'];
		...
		generateOutput: function ( $target, settings ) {
			...
			if ( typeof settings['enable_offset'] !== 'undefined' && settings['enable_offset'] === 'offset' && typeof settings['offset_image_url'] !== 'undefined' ) {
				...
				var img = '<img src="' + settings['offset_image_url'] + '" alt="Offset Image">';

				if ( typeof settings['offset_retina'] !== 'undefined' && settings['offset_retina'] === 'yes' ) {
					 img = '<img style="transform:scale(0.5);" src="' + settings['offset_image_url'] + '" alt="Offset Image">';
				}

				$( '<div class="qodef-offset-image-holder" style="position: absolute; z-index: 5; ' + settings['offset_vertical_anchor'] + ':' + settings['offset_vertical_position'] + ';' + settings['offset_horizontal_anchor'] + ':' + settings['offset_horizontal_position'] + ';' + 'max-width' + ':' + settings['offset_max_width'] + '"><div class="qodef-offset-image-wrapper' + $parallaxClass + '">'+img+'</div></div>' ).prependTo( $target );
			}
		}
	};
				...
				$.each(
					...
					function ( index, property ) {
						...
						if ( typeof $target !== 'undefined' && $target.length ) {
							qodefElementorColumnCallback.generateOutput(
								$target,
								settings
							);
						}
	...
	var qodefElementorContainerCallback = {
		init: function ( $scope ) {
			...
			if ( isEditMode && typeof $scope !== 'undefined' ) {
				...
				if ( typeof containerData.qodef_offset_image !== 'undefined' && containerData.qodef_offset_image['url'] ) {
					settings['offset_image_url'] = containerData.qodef_offset_image['url'];
				}

				if ( typeof containerData.qodef_offset_parallax !== 'undefined' ) {
					settings['offset_parallax'] = containerData.qodef_offset_parallax;
				}

				if ( typeof containerData.qodef_offset_vertical_anchor !== 'undefined' ) {
					settings['offset_vertical_anchor'] = containerData.qodef_offset_vertical_anchor;
				}

				if ( typeof containerData.qodef_offset_vertical_position !== 'undefined' ) {
					settings['offset_vertical_position'] = containerData.qodef_offset_vertical_position;
				}

				if ( typeof containerData.qodef_offset_horizontal_anchor !== 'undefined' ) {
					settings['offset_horizontal_anchor'] = containerData.qodef_offset_horizontal_anchor;
				}

				if ( typeof containerData.qodef_offset_horizontal_position !== 'undefined' ) {
					settings['offset_horizontal_position'] = containerData.qodef_offset_horizontal_position;
				}
