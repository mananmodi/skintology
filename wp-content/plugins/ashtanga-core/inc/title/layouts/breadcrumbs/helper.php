...
if ( ! function_exists( 'ashtanga_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function ashtanga_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'AshtangaCore_Breadcrumbs_Title';

		return $layouts;
	}

	add_filter( 'ashtanga_core_filter_register_title_layouts', 'ashtanga_core_register_breadcrumbs_title_layout' );
}

if ( ! function_exists( 'ashtanga_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function ashtanga_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'ashtanga-core' );

		return $layouts;
	}

	add_filter( 'ashtanga_core_filter_title_layout_options', 'ashtanga_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'ashtanga_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function ashtanga_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();

		// Breadcrumbs label
		$labels = apply_filters(
			'ashtanga_core_filter_breadcrumbs_label',
			array(
				'home'        => esc_html__( 'Home', 'ashtanga-core' ),
				'tag'         => esc_html__( 'Posts tagged "%s"', 'ashtanga-core' ),
				'author'      => esc_html__( 'Posted by %s', 'ashtanga-core' ),
				'search'      => esc_html__( 'Search results for "%s"', 'ashtanga-core' ),
				'404'         => esc_html__( '404 - Page not found', 'ashtanga-core' ),
				'query_paged' => esc_html__( '(Page %s)', 'ashtanga-core' ),
			)
		);

		// Breadcrumbs variables
		$settings = apply_filters(
			'ashtanga_core_filter_breadcrumbs_settings',
			array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'ashtanga_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}

		echo apply_filters( 'ashtanga_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}
