<div <?php qode_framework_class_attribute( $holder_classes ); ?>>
	<?php foreach ( $params['working_hours_params'] as $day => $time ) : ?>
		<div class="qodef-working-hours-item qodef-e">
			<?php if ( ! empty( $day ) ) : ?>
				<h6 class="qodef-e-day"><?php echo esc_html( $day ); ?>
					<?php
					foreach ( $params['working_hours_special_params']['special_days'] as $special ) :
						if ( $day === $special ) :
							echo ashtanga_core_get_svg_icon( 'star', 'qodef-e-day-icon' );
						endif;
					endforeach;
					?>
				</h6>
			<?php endif; ?>
			<div class="qodef-e-line"></div>
			<?php if ( ! empty( $time ) ) { ?>
				<h6 class="qodef-e-time"><?php echo esc_html( $time ); ?></h6>
			<?php } else { ?>
				<h6 class="qodef-e-time qodef--closed"><?php esc_html_e( 'Closed', 'ashtanga-core' ); ?></h6>
			<?php } ?>
		</div>
	<?php endforeach; ?>
	<div class="qodef-m-footer">
		<?php if ( isset( $params['working_hours_special_params']['special_text'] ) && ! empty( $params['working_hours_special_params']['special_text'] ) ) : ?>
			<span class="qodef-m-footer-label"><?php echo esc_html( $params['working_hours_special_params']['special_text'] ); ?></span>
		<?php endif; ?>
		<?php echo ashtanga_core_get_svg_icon( 'star', 'qodef-m-footer-icon' ); ?>
	</div>
</div>
