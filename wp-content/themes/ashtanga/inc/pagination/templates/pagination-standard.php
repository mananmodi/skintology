<?php if ( isset( $query_result ) && intval( $query_result->max_num_pages ) > 1 ) {
	...
	$dots       = '<span class="qodef-m-pagination-item qodef--dots">' . esc_html__( '&hellip;', 'ashtanga' ) . '</span>';
	$set_dots   = false;
	$dots_added = false;
	...
	?>
	<div class="qodef-m-pagination qodef--standard" <?php ashtanga_is_installed( 'framework' ) && isset( $pagination_top_margin ) ? qode_framework_inline_style( $pagination_top_margin ) : ''; ?>>
		<div class="qodef-m-pagination-inner">
			<nav class="qodef-m-pagination-items">
				<a class="qodef-m-pagination-item qodef--prev <?php echo esc_attr( implode( ' ', $prev_classes ) ); ?>" href="#" data-paged="1">
					<?php ashtanga_render_svg_icon( 'pagination-arrow-left' ); ?>
				</a>
				<?php
				for ( $i = 1; $i <= $total; $i ++ ) {
					$classes     = array();
					...
					if ( ! $set_dots && $total > ( $mid_size + 1 ) ) {
						...
						if ( 1 === $current && ( $mid_size + 1 ) === $i ) {
							...
							if ( $i <= ( $current - $mid_size ) && ! in_array( $i, array( 1, $total ), true ) ) {
								...
								// Added dot's markup between first item and current item
				...
				<a class="qodef-m-pagination-item qodef--next <?php echo esc_attr( implode( ' ', $next_classes ) ); ?>" href="#" data-paged="2">
					<?php ashtanga_render_svg_icon( 'pagination-arrow-right' ); ?>
				</a>
			</nav>
		</div>
	</div>
	...
	ashtanga_render_svg_icon( 'spinner', 'qodef-m-pagination-spinner' );
	?>
<?php } ?>
