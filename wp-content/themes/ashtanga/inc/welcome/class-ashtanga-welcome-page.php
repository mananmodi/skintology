...
if ( ! class_exists( 'Ashtanga_Welcome_Page' ) ) {
	/**
	 * Welcome page theme class
	 */
	class Ashtanga_Welcome_Page {
		private static $instance;

		/**
		 * @return Ashtanga_Welcome_Page
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_ashtanga_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_ashtanga_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_ashtanga_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'ashtanga_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			add_theme_page(
				esc_html__( 'About', 'ashtanga' ),
				esc_html__( 'About', 'ashtanga' ),
				current_user_can( 'edit_theme_options' ),
				'ashtanga_welcome_page',
				array( $this, 'welcome_page_content' )
			);

			remove_submenu_page( 'themes.php', 'ashtanga_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( ASHTANGA_ROOT_DIR . '/screenshot.png' ) ? ASHTANGA_ROOT . '/screenshot.png' : ASHTANGA_ROOT . '/screenshot.jpg';

			ashtanga_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_ashtanga_welcome_page' === $hook ) {
				wp_enqueue_style( 'ashtanga-welcome-page-style', ASHTANGA_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Ashtanga_Welcome_Page::get_instance();
}
