...
if ( ! function_exists( 'ashtanga_get_filter_items' ) ) {
	/**
	 ...
	 * @return array
	 */
	function ashtanga_get_filter_items( $params ) {
		...
		if ( isset( $params['tax_slug'] ) && ! empty( $params['tax_slug'] ) ) {
			...
			if ( isset( $specific_taxonomy->term_id ) && ! empty( $specific_taxonomy->term_id ) ) {
		...
		return apply_filters( 'ashtanga_filter_get_filter_items', $items, $params );
	}
}
