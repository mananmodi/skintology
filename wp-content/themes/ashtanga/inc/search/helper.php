...
if ( ! function_exists( 'ashtanga_get_search_page_excerpt_length' ) ) {
	/**
	 * Function that return number of characters for excerpt on search page
	 *
	 * @return int
	 */
	function ashtanga_get_search_page_excerpt_length() {
...
if ( ! function_exists( 'ashtanga_override_search_block_templates' ) ) {
	/**
	 * Function that override `core/search` block template
	 *
	 * @see register_block_core_search()
	 */
	function ashtanga_override_search_block_templates( $atts ) {
		if ( ! empty( $atts ) && isset( $atts['render_callback'] ) && 'render_block_core_search' === $atts['render_callback'] && function_exists( 'styles_for_block_core_search' ) ) {
			$atts['render_callback'] = 'ashtanga_render_block_core_search';
		}

		return $atts;
	}

	add_filter( 'block_type_metadata_settings', 'ashtanga_override_search_block_templates' );
}

if ( ! function_exists( 'ashtanga_render_block_core_search' ) ) {
	/**
	 ...
	 */
	function ashtanga_render_block_core_search( $attributes ) {
		static $instance_id = 0;

		$attributes = wp_parse_args(
			$attributes,
			array(
				'label'      => esc_html__( 'Search', 'ashtanga' ),
				'buttonText' => esc_html__( 'Search', 'ashtanga' ),
			)
		);
		...
		$label_markup = sprintf(
			...
			$input_id,
			empty( $attributes['label'] ) ? esc_html__( 'Search', 'ashtanga' ) : esc_html( $attributes['label'] )
		);
		...
		if ( $show_button ) {
			...
			if ( ! $use_icon_button ) {
				...
				$button_internal_markup = ashtanga_get_svg_icon( 'search' );
			}
