(function ( $ ) {
	...
	$( document ).ready(
		function () {
			...
			qodefAnchor.init();
			qodefModifiedSimpleButton.init();
		}
	);
	...
	var qodefSwiper = {
		...
		initSlider: function ( $currentItem ) {
			...
			if ( $currentItem.hasClass( 'qodef-testimonials-list' ) ) {
				qodefSwiper.swipers['testimonials'].push( new Swiper(
					$currentItem[0],
					Object.assign(
						options,
						events
				...
				qodefSwiper.swipers['other'].push( new Swiper(
					$currentItem[0],
					Object.assign(
						options,
						events
		...
		getOptions: function ( $holder ) {
			...
			if ( sliderOptions.slidesPerView1440 !== undefined && sliderOptions.slidesPerView1440 !== '' ) {
				slidesPerView1440 = 'auto' === sliderOptions.slidesPerView1440 ? 'auto' : parseInt( sliderOptions.slidesPerView1440, 10 );
			}

			if ( sliderOptions.slidesPerView1366 !== undefined && sliderOptions.slidesPerView1366 !== '' ) {
				slidesPerView1366 = 'auto' === sliderOptions.slidesPerView1366 ? 'auto' : parseInt( sliderOptions.slidesPerView1366, 10 );
			}
			...
			if ( sliderOptions.slidesPerView768 !== undefined && sliderOptions.slidesPerView768 !== '' ) {
				slidesPerView768 = 'auto' === sliderOptions.slidesPerView768 ? 'auto' : parseInt( sliderOptions.slidesPerView768, 10 );
			}
			...
			if ( sliderOptions.slidesPerView480 !== undefined && sliderOptions.slidesPerView480 !== '' ) {
				slidesPerView480 = 'auto' === sliderOptions.slidesPerView480 ? 'auto' : parseInt( sliderOptions.slidesPerView480, 10 );
			}
			...
			var options = {
				...
				pagination: {
					...
					renderBullet: function (index, className) {
					  	return '<span class="' + className + '"><svg class="qodef-svg--bullet" xmlns="http://www.w3.org/2000/svg" width="12.728" height="12.728" viewBox="0 0 12.728 12.728"><g transform="translate(6.364) rotate(45)" fill="none" stroke-width="1"><rect width="9" height="9" stroke="none"/><rect x="0.5" y="0.5" width="8" height="8" fill="none"/></g></svg></span>';
					},
				},
				breakpoints: {
			...
			return {
				on: {
					...
					init: function () {
						...
						qodef.body.trigger(
							'ashtanga_trigger_swiper_is_initialized',
							[$holder, options]
						);
	...
	/*
	 *  Add html to various simple buttons
	 */
	var qodefModifiedSimpleButton = {
		init: function () {
			var $simpleButton = $('.price_slider_amount .button');
			qodefModifiedSimpleButton.addHtml($simpleButton);
		},

		addHtml: function (button, text = ''){
			if (button.length) {
				button.each(function () {
					var thisButton = $(this),
						buttonText = (typeof (text) === 'string' && text !== '') ? text : thisButton.text(),
						buttonAdditional  = '<svg class="qodef-svg--button-arrow qodef-m-arrow" xmlns="http://www.w3.org/2000/svg" width="37.833" height="18.959" viewBox="0 0 37.833 18.959"><path fill="none" stroke="currentColor" d="M37.828 9.371H0"/><path fill="none" stroke="currentColor" d="M37.83 9.48S28.651 9.403 26.143.131"/><path fill="none" stroke="currentColor" d="M37.83 9.479s-9.179.077-11.687 9.349"/></svg>';

					thisButton.html('<span class="qodef-m-text">' + buttonText + '</span>' + buttonAdditional );
				});
			}
		}
	};

	qodef.qodefModifiedSimpleButton = qodefModifiedSimpleButton;
...
(function ( $ ) {
	...
	$( document ).ready(
		function () {
			...
			$( '.woocommerce-product-details__short-description' ).each(
				function () {
					if ( $( this ).height() > 44 ) {
						$( this ).addClass( 'expand-description' );
					}
				}
			);
			$( '.expand-description' ).on(
				'click',
				function () {
					$( this ).toggleClass( 'qodef-fullheight' );
				}
			);
		}
	);
	...
	$( window ).on(
		...
		function () {
			...
			qodefWooButtonPriceFilter.init();
		}
	);
	...
	var qodefWooButtonPriceFilter = {
		init: function () {
			const $holder = $( '.wc-block-components-price-slider' );
					...
					function () {
						const $thisHolder = $( this ),
							  interval    = setInterval(
								  function () {
									  if ( ! $thisHolder.hasClass( 'is-loading' ) ) {
										  const $button = $thisHolder.find( '.wc-block-filter-submit-button' );

										  if ( $button.length ) {
											  qodef.qodefModifiedSimpleButton.addHtml(
												  $button,
												  $button.find( '> span:first-child' ).text()
											  );
										  }
